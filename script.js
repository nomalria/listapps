...
function startEditList(listId) {
    ...
    editContainer.innerHTML = `
        ...
        <div class="edit-buttons">
            <button class="save-btn" id="save-list-${listId}">저장</button>
            <button class="cancel-btn" id="cancel-list-${listId}">취소</button>
        </div>
    `;
    ...
    // 이벤트 리스너 추가
    document.getElementById(`save-list-${listId}`).addEventListener('click', function(e) {
        e.stopPropagation();
        saveListEdit(listId);
    });
    
    document.getElementById(`cancel-list-${listId}`).addEventListener('click', function(e) {
        e.stopPropagation();
        cancelListEdit(listId, originalTitle);
    });
...
function startEditMemo(listId, memoId) {
    ...
    editContainer.innerHTML = `
        ...
        <div class="edit-buttons">
            <button class="save-btn" id="save-memo-${memoId}">저장</button>
            <button class="cancel-btn" id="cancel-memo-${memoId}">취소</button>
        </div>
    `;
    
    // 기존 내용 대체
    memoElement.innerHTML = '';
    memoElement.appendChild(editContainer);
    
    // 이벤트 리스너 추가
    document.getElementById(`save-memo-${memoId}`).addEventListener('click', function(e) {
        e.stopPropagation();
        saveMemoEdit(listId, memoId);
    });
    
    document.getElementById(`cancel-memo-${memoId}`).addEventListener('click', function(e) {
        e.stopPropagation();
        cancelMemoEdit(listId, memoId, originalText);
    });
